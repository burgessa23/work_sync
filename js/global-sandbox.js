...
$(window).on("load", function() {
	...
	if($('.slideBar').length != 0)	  				{ $.getScript('//www.att.com/styles/att/assets3.1.0/js/jquery.nouislider.js', $.fn.slider, true); }
	...
	if($('.flexslider').length != 0)					{ $.getScript('//www.att.com/styles/att/assets3.1.0/js/jquery.flexslider.js', $.fn.marquee, true); }
	...
	$('.progress > [data-percentage]').each(function(){
		...
		$(this).animate({
			...
			$(this).append('<span class="hidden-spoken">'+barWidth+'%</span>');
		});
	});
    ...
    $target
      ...
      .one('hide', function () {
		...
		$('.modal').on('shown', function () {
		  var headHeight = $(this).find('.modal-header').height()+1;
		  var footHeight = $(this).find('.modal-footer').height()+51;
		  var modalHeight = $(this).height();
		  var bodyHeight = modalHeight - (headHeight + footHeight) + 'px';
...
tabpanel.prototype.togglePanel = function($tab) {
  ...
  if ($panel.attr('aria-hidden') == 'true') {
    ...
    $tab.find('i').removeClass().addClass('cssIcon-toggle-minus').html('expanded');
	 $('body').attr('data-scroll-position', $(document).scrollTop());
	 $("html, body").animate({scrollTop:$tab.offset().top},1000)
    ...
    $("html, body").animate({scrollTop:$('body').attr('data-scroll-position')},50)
